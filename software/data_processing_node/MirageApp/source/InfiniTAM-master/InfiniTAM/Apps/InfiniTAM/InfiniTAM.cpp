...
static ITMMainEngine *CreateMainEngine(const ITMLibSettings *internalSettings, ImageSourceEngine *imageSource)
{
	switch (internalSettings->libMode)
	{
	case ITMLibSettings::LIBMODE_BASIC:
		return new ITMBasicEngine<ITMVoxel, ITMVoxelIndex>(internalSettings, imageSource->getCalib(), imageSource->getRGBImageSize(), imageSource->getDepthImageSize());
	case ITMLibSettings::LIBMODE_BASIC_SURFELS:
		return new ITMBasicSurfelEngine<ITMSurfelT>(internalSettings, imageSource->getCalib(), imageSource->getRGBImageSize(), imageSource->getDepthImageSize());
	case ITMLibSettings::LIBMODE_LOOPCLOSURE:
		return new ITMMultiEngine<ITMVoxel, ITMVoxelIndex>(internalSettings, imageSource->getCalib(), imageSource->getRGBImageSize(), imageSource->getDepthImageSize());
	default:
...
try
{
	...
	if (arg == 1)
	{
		printf("starting D415 GUI mode. Use Apply & Initialize Camera in the left panel.\n");
		imageSource = new BlankImageGenerator(arg1, Vector2i(848, 480));
	}
	else
	{
	...
	ITMMainEngine *mainEngine = CreateMainEngine(internalSettings, imageSource);

	UIEngine::Instance()->Initialise(argc, argv, imageSource, imuSource, mainEngine, "./Files/Out", internalSettings, arg1);
